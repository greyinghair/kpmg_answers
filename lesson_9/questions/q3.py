# Q3
# Create a function that takes a file name and word. Search the file to find all lines that start with the given word.
#
...
def find(word, file):
    ...
    for x in ignore: # loop for each entry in ignore list
        for line in file: # loop for each line in the file
            if line.startswith(word) or line.startswith(x + word):
                print(line) # show the line found to match on each loop
